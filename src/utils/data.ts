  ...
  jobs: [
    {
      ...
      summary: {
        en: "A staff PMS for cabin and apartment hospitality: nightly, monthly, and yearly stays, walk-in and OTA desks, and on-site money — without a channel manager.",
        ch: "给民宿和公寓前台用的物业管理系统：按晚、按月、按年入住，接待散客和 OTA 订单，现场收钱，不接渠道管理器。",
      },
      bullets: {
        en: [
          "Built a NestJS REST API and PostgreSQL PMS for 100+ units so the front desk can book nightly, monthly, and yearly stays, check guests in and out, and sync Booking.com, Airbnb, and Agoda calendars without a channel manager.",
          "Blocked overlapping stays and calendar holds in PostgreSQL so two confirmed bookings cannot share the same unit-dates, and kept monthly and yearly stays occupying the unit until checkout.",
          "Billed stay totals as rent plus electricity, water, and maintenance from monthly meter readings, so long-stay quotes are a rollup rather than one typed-in amount.",
          "Recorded deposits, top-ups, and refunds as append-only payment movements so the quote and cash received stay separate — staff cannot silently edit past cash events.",
          "Imported OTA iCal feeds on a schedule and on demand, opening unconfirmed bookings and a mismatch queue when dates or units disagree with the local calendar.",
        ],
        ch: [
          "用 NestJS REST API 和 PostgreSQL 做了一套覆盖 100+ 间房的物业管理系统，前台能订按晚、按月、按年的入住，办理入住退房，并同步 Booking.com、Airbnb、Agoda 的日历，不需要渠道管理器。",
          "在 PostgreSQL 里拦住重叠入住和日历占用，两笔已确认订单不能占同一间房的同一段日期；按月、按年的入住会一直占房直到退房。",
          "长住报价按房租加上电费、水费、维护费，从每月抄表滚出来，而不是前台手填一个总数。",
          "押金、充值和退款记成只能追加的资金流水，报价和实收分开，前台改不了已经记下的收付款。",
          "按计划和按需导入 OTA 的 iCal，日期或房号对不上本地日历时，会开出未确认订单并进差异队列。",
        ],
      },
      stack: [
        "TypeScript",
    ...
    },
    {
      ...
      summary: {
        en: "Consumer AI roleplay chat with UGC creators. I worked on the web app at chat.mufy.ai — discovery, commerce, and account.",
        ch: "面向消费者的 AI 角色扮演聊天，创作者可以上传自己的角色。我在 chat.mufy.ai 做 Web 端，覆盖发现、交易和账号。",
      },
      bullets: {
        en: [
          "Shipped 15+ production pages on chat.mufy.ai for consumers and UGC creators, covering discovery, commerce, and account.",
          "Built role and creator feeds (recommend, latest, following, trending) and threaded comments with author pin and fan-badge display.",
          "Delivered creator commerce screens: cosmetic shops, wallet top-up, and red-packet (cash gift) distribution.",
          "Pulled fan-badge and comment-input into shared components so settings and backpack stopped shipping duplicate UI.",
          "Replaced a stored session token with a short-lived access token plus an httpOnly refresh cookie, sharing one refresh in flight on app boot and on 401 responses.",
          "Migrated a second web client at the same company from Ant Design to Radix, Tailwind, React Hook Form, and Zod.",
        ],
        ch: [
          "在 chat.mufy.ai 上线了 15+ 个面向消费者和创作者的正式页面，覆盖发现、交易和账号。",
          "做了角色和创作者信息流（推荐、最新、关注、热门），以及带作者置顶和粉丝徽章的楼层评论。",
          "做了创作者侧的交易界面：装扮商店、钱包充值，以及红包发放。",
          "把粉丝徽章和评论输入抽成共用组件，设置和背包不再各写一套同样的界面。",
          "把本地存的会话 token 换成短时 access token 加 httpOnly 刷新 cookie，启动和 401 时共用同一次刷新，不会并发打两次。",
          "把公司另一个 Web 客户端从 Ant Design 迁到 Radix、Tailwind、React Hook Form 和 Zod。",
        ],
      },
      stack: [
        "React",
